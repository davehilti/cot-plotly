...
token = "yj5TmTBVIUvIKkyUQF4-CIfh-sECcasv2p6Is_VODcn50Z3Wg9SfmNDZDIjGmVeFxQi27yJzelmn0ISNP6EXPQ=="
org = "cot-plotly"
bucket = "CoT-Data"
...
df['_time'] = pd.to_datetime(df['_time']).dt.tz_localize(None)  # Entferne Zeitzoneninformationen

# Fülle NaN-Werte oder entferne Zeilen mit NaN 
df.fillna(0, inplace=True)  # Füllt NaN mit 0

# Close the client
client.close()

# Ensure the _time column (Date) is in datetime format
df['_time'] = pd.to_datetime(df['_time'])

# Filter the dataframe for valid timestamps within the allowed retention period
valid_start_date = pd.to_datetime('2024-09-20')
df['_time'] = df['_time'].dt.tz_localize(None)
df_filtered = df[df['_time'] >= valid_start_date]

# Pivot the dataframe using the filtered data
df_pivoted = df_filtered.pivot_table(values='_value', index=['_time', 'market_names'], columns='_field').reset_index()

# Rename columns for convenience
df_pivoted.columns.name = None
df_pivoted.rename(columns={'_time': 'Date', 'market_names': 'Market Names'}, inplace=True)

# Calculate clustering for long and short positions
df_pivoted['Long Clustering'] = df_pivoted['Producer/Merchant/Processor/User Long'] / df_pivoted['Producer/Merchant/Processor/User Long'].sum() * 100
df_pivoted['Short Clustering'] = df_pivoted['Producer/Merchant/Processor/User Short'] / df_pivoted['Producer/Merchant/Processor/User Short'].sum() * 100
...
def update_graphs(selected_market, start_date, end_date, mm_type, trader_group):
    ...
    long_clustering_fig.add_trace(go.Scatter(
        ...
        marker=dict(
            ...
            colorscale='Viridis',
            showscale=True,
            colorbar=dict(title='Long Clustering (%)')
        ),
        text=filtered_df['Total Number of Traders'],  # Tooltip with number of traders
        name='Long Positions Clustering'
    ))
    long_clustering_fig.update_layout(
        title='Long Positions Clustering',
        xaxis_title='Date',
    ...
    short_clustering_fig.add_trace(go.Scatter(
        ...
        marker=dict(
            ...
            colorscale='Viridis',
            showscale=True,
            colorbar=dict(title='Short Clustering (%)')
        ),
        text=filtered_df['Total Number of Traders'],  # Tooltip with number of traders
        name='Short Positions Clustering'
    ))
    short_clustering_fig.update_layout(
        title='Short Positions Clustering',
        xaxis_title='Date',
        yaxis_title='Open Interest'
    )
    ...
    long_position_size_fig.add_trace(go.Scatter(
        ...
        mode='markers',
        marker=dict(
            size=filtered_df['Long Position Size'] / long_scaling_factor,  # Dynamically adjusted size
            color=filtered_df['Total Long Traders'],
            colorscale='Viridis',
            showscale=True,
            colorbar=dict(title='Number of Long Traders')
        ),
        text=filtered_df['Long Position Size'],  # Tooltip with position size
        name='Long Position Size'
    ))
    ...
    short_position_size_fig.add_trace(go.Scatter(
        ...
        mode='markers',
        marker=dict(
            size=filtered_df['Short Position Size'] / short_scaling_factor,  # Dynamically adjusted size
            color=filtered_df['Total Short Traders'],
            colorscale='Viridis',
            showscale=True,
            colorbar=dict(title='Number of Short Traders')
        ),
        text=filtered_df['Short Position Size'],  # Tooltip with position size
        name='Short Position Size'
    ))
    ...
    max_bubble_size = 30  # Adjusted for better visualization
